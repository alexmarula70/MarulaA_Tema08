...
namespace OpenTK_winforms_z02
{
    public partial class Form1 : Form
    {
        ...
        private void SetupValues() {
            ...
            setLight0Values();
            light1 = new Light(LightName.Light1, valuesAmbientTemplate1, valuesDiffuseTemplate1, valuesSpecularTemplate1, valuesPositionTemplate1);
        ...
        public void setSource1Status(bool status)
        {
            if (status == false)
            {
                light1.On = false;
        ...
        {
            light1.Position[0] = trackLight1PositionX.Value;
            GlControl1.Invalidate();
        }

        private void trackLight1PositionY_Scroll(object sender, EventArgs e)
        {
            light1.Position[1] = trackLight1PositionY.Value;
            GlControl1.Invalidate();
        }

        private void trackLight1PositionZ_Scroll(object sender, EventArgs e)
        {
            light1.Position[2] = trackLight1PositionZ.Value;
            GlControl1.Invalidate();
        }

        private void setLight1IntensityDefault()
        {
            light1Intensity.Value = 255;
            GlControl1.Invalidate();
        }
        ...
        private void light1Intensity_ValueChanged(object sender, EventArgs e)
        {
            float intensity = (float)light1Intensity.Value / 255;

            light1.Diffuse = new float[] { intensity, intensity, intensity, 1.0f };
            light1.Specular = new float[] { intensity, intensity, intensity, 1.0f };
            light1.Ambient = new float[] { intensity, intensity, intensity, 1.0f };
            GlControl1.Invalidate();
